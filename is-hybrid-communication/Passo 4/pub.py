...
#Ouvindo a aplicação de comunicação híbrida
hc = 0
def listen_is_hybrid_communication():
    subscription = Subscription(channel)
    subscription.subscribe(topic="Hybrid.Communication.Topic.Frame")
    log.info('Subscribed to Hybrid.Communication')
    while True:
        msg_hc = channel.consume()
        hc = msg_hc[0]
        HOST = msg_hc[1]
        PORT = msg_hc[2]
...
thread = threading.Thread(target=listen_is_hybrid_communication, daemon=True)
thread.start()
