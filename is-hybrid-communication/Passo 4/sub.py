...
if __name__ == '__main__':
    ...
    while True:
        log.info(f"Waiting for Messages")

        if aux<10:
            msg = channel.consume()
            timestamp_rcvd = time.time()
            log.info('Message received via PUB/SUB')
        else:
            data, addr = server_socket.recvfrom(65535)
            timestamp_rcvd = time.time()
            msg = pickle.loads(data)
            log.info('Message received via Socket')

        msg_hc = ('Topic.Frame',HOST,PORT)

        channel.publish(msg_hc.encode('latin'), topic = 'Hybrid.Communication.Consumer')
        ...
        log.info("Sending message from Contrace")
        UDPClientSocket.sendto(bytesToSend, serverAddressPort)
        log.info("Message from Contrace: {}", msg_contrace)
